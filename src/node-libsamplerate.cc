...
SampleRateStream::SampleRateStream(const Napi::CallbackInfo &info) : Napi::ObjectWrap<SampleRateStream>(info) {
...
Napi::Value SampleRateStream::Transform(const Napi::CallbackInfo &info) {
  ...
  if (fromDepth == 16 && toDepth != 16) lengthOut = lengthOut * 2;
  ...
  if (frameDiff != 0)
    printf("Outframes differs from inframes by %d.\n", frameDiff);
